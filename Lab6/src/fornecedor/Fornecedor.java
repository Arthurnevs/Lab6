...
public class Fornecedor {
	...
	public void adicionaCombo(String nome, String descricao, double fator, String nomeProdutos) {
		String key = nome + descricao;
		double preco = 0;
		...
		for(String produto : nomeProdutos.split(",")) {
			Produto encontrado = this.produtos.get(produto.split(" - ")[0].trim() + produto.split(" - ")[1].trim());
			if(encontrado == null) {
				throw new IllegalArgumentException("Erro no cadastro de compo: produtoao existe.");
			}
			else {
				preco += encontrado.getPreco();
			}
			
			String chave = encontrado.getNome() + encontrado.getDescricao();
			produtos.put(chave, encontrado);
		}
		this.produtos.put(key, new Combo(nome, descricao,preco,fator, produtos));
		this.produtosCadastrados.add(key);
	...
	public void editaCombo(String nome,String descricao,double novoFator) {
		...
		this.produtos.get(key).modificaCombo(novoFator);
	}
